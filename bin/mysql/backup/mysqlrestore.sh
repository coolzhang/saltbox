...
if [ "$?" = "0" ];then
  ...
  sleep 10
  ${MYSQL} -uroot -S${socket} -e "set password=password('${password}');set global slow_query_log=0;DELETE FROM mysql.user WHERE User='';DELETE FROM mysql.user WHERE User='root' AND Host NOT IN ('localhost');DROP DATABASE IF EXISTS test;DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%';FLUSH PRIVILEGES;grant replication slave, replication client on *.* to 'repl'@'%' identified by 'repl';GRANT SELECT, SUPER, PROCESS, REPLICATION CLIENT ON *.* TO 'mpm'@'127.0.0.1' identified BY 'pass4mpm';grant select on *.* to restore@localhost identified by 'YY18iRSUBoIcrpp4d4Vor78QT';grant all on *.* to admin@'10.%' identified by '78q3letSPQGuAwDG1Meq6yO5O';GRANT SELECT, INSERT, UPDATE, DELETE ON *.* TO 'anemometer'@'%' IDENTIFIED BY 'UiKNK0g73l8St7FxNFYY6OaQx';grant select on *.* to bigdata@'%' identified by 'Gn_jNErfzfCEQ5oAePUPV8irQ';grant select on *.* to techs@'%' identified by 'napRGi_Juoakr_UXBwoXDXnru'" >/dev/null 2>&1 |grep -v "Logging to file"
fi
