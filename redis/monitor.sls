...
{% for conf in ['userparams_connect-trapper.conf','userparams_redis-lld.conf'] %}
{{ conf }}:
  file.managed:
    - name: /etc/zabbix/zabbix_agentd.d/{{ conf }}
    - source: salt://conf/zabbix/{{ conf }}
    - user: zabbix
    - group: zabbix
    - mode: 644
    - require:
...
{% for script in ['Redis-info','Redis-port-lld','tcp_trapper_connect.sh'] %}
{{ script }}:
  file.managed:
...
sed-tcp_trapper_connect.sh:
  cmd.run:
    - name: sed -i -e 's/zabbix_agent_hostname/{{ grains['id'].rsplit('-',1)[0] }}/' -e 's/zabbix_server_ip/{{ zabbix_server }}/' /etc/zabbix/script/tcp_trapper_connect.sh
    - require:
